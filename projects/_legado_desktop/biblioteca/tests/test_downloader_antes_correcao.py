...
def test_baixar_ytdlp_sucesso(tmp_path, monkeypatch):
    ...
    class YoutubeDLFake:
        ...
        def extract_info(self, url, download=True):
            ...
            caminho = Path(self.opcoes["outtmpl"].replace(
                "%(title)s.%(ext)s",
                "video.mp4"
            ))
...
def test_baixar_ytdlp_usa_filename_quando_filepath_nao_existe(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert "erro simulado" in str(erro)
    else:
        raise AssertionError("O erro do yt-dlp deveria ser propagado.")
...
def test_baixar_ytdlp_sem_arquivo_final(tmp_path, monkeypatch):
    ...
    try:
        ...
        raise AssertionError(
            "O downloader deveria detectar que nenhum arquivo foi produzido."
        )
...
def test_baixar_ytdlp_cancelado(tmp_path, monkeypatch):
    ...
    class YoutubeDLFake:
        ...
        def extract_info(self, url, download=True):
            raise DownloadCancelled("Download cancelado pelo usuÃ¡rio.")
